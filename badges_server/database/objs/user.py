...
class User(baseobjc, CreateableMixin, UUIDCreateableMixin, UserCreateableMixin):
    ...
    granting = relationship(Granting, backref="user", passive_deletes=True)
    invitation = relationship(Invitation, backref="user", passive_deletes=True)
    provider = relationship(Provider, backref="user", passive_deletes=True)
